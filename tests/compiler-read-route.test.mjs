...
test("compiler read route returns compilerRead payload on success", async () => {
  const compilerRead = {
    ...
    translatedSubsetResult: {
      present: true,
      source: 'GND box @doc\nDIR aim "Check the protocol."\nMOV move "Check the protocol." via manual',
      translationStrategy: "Translated one protocol claim into a minimal lawful subset.",
      omittedClaims: [],
    ...
    outcomeClass: "mixed",
    verdict: { overall: "lawful_subset_compiles", primaryFinding: "Found it.", readDisposition: "needs_more_witness" },
