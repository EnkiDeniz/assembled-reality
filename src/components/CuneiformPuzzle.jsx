...
export default function CuneiformPuzzle({ onSolved }) {
  ...
  const phaseLines = useMemo(() => {
    ...
    return PUZZLE_SUCCESS_PHASES[state.unlockPhase] ||
      PUZZLE_SUCCESS_PHASES[PUZZLE_SUCCESS_PHASES.length - 1];
  }, [state.unlockPhase]);
    ...
    <div className="matrix-shell">
      ...
      <div className="matrix-grid" role="grid" aria-label="Cuneiform lock">
        {state.grid.map((column, columnIndex) => (
          <div
            key={columnIndex}
            className={`matrix-column ${state.frozenColumns[columnIndex] ? "is-frozen" : ""}`}
          >
