...
function getProjectForDocumentKey(projects = [], documentKey = "") {
  ...
  return (
    (Array.isArray(projects) ? projects : []).find(
      (project) =>
        Array.isArray(project?.documentKeys) &&
        project.documentKeys.includes(normalizedDocumentKey),
    ) || null
  );
}
...
function getProjectDocumentCandidates(project = null, documents = []) {
  ...
  const visibleKeys = new Set(
    visibleProjectDocuments.map((document) => document.documentKey),
  );
...
function getMostRecentWorkspaceProject(projects = [], documents = []) {
  ...
  const nonExampleProjects = normalizedProjects.filter(
    (project) => !isSystemExampleProject(project),
  );
  ...
  const rankedCandidates = documentBearingCandidates.length
    ? documentBearingCandidates
    : candidates;
  return (
...
function getFirstDocumentBearingProject(projects = [], documents = []) {
  const normalizedProjects = Array.isArray(projects) ? projects.filter(Boolean) : [];
  return (
    normalizedProjects.find((project) => hasDocumentBearingProject(project, documents)) ||
    null
  );
}
  ...
  return (
    [
      ...listRealDocuments(documents).filter((document) =>
        Array.isArray(project?.documentKeys)
          ? project.documentKeys.includes(document.documentKey)
          : false,
      ),
    ].sort((left, right) => getDocumentTimestamp(right) - getDocumentTimestamp(left))[0]
      ?.documentKey || ""
  );
}
...
function isMobileRequest(userAgent = "") {
  ...
  return /iphone|ipad|ipod|android|mobile|blackberry|opera mini|windows phone/.test(
    normalized,
  );
}
...
async function measureWorkspaceStep(label, action) {
  ...
  try {
    ...
    console.error(
      `[workspace] ${label} failed after ${Date.now() - startedAt}ms`,
      error,
    );
    throw error;
  }
...
export async function loadWorkspacePageData({
  ...
  const session = await measureWorkspaceStep("getRequiredSession", () =>
    getRequiredSession(),
  );
  if (!session?.user?.id) {
    redirect("/");
  ...
  const requestedLaunchpadView = String(resolvedSearchParams?.launchpadView || "")
    .trim()
    .toLowerCase();
  ...
  const connectedIntegration = String(resolvedSearchParams?.connected || "")
    .trim()
    .toLowerCase();
  ...
  const initialWorkspaceNotice =
    connectedIntegration === "getreceipts"
      ? {
          message:
            "Connected. Your next sealed receipt will be stamped by GetReceipts.",
          tone: "success",
        }
      : integrationError
        ? {
  ...
  const exampleBootstrap = await measureWorkspaceStep(
  ...
  const hasExampleDocuments = documents.some((document) =>
    isSystemExampleDocument(document),
  );
  if (
    exampleBootstrap.refreshed ||
    (exampleBootstrap.hasExampleProject && !hasExampleDocuments)
  ) {
    documents = await measureWorkspaceStep(
  ...
  const isFirstTime =
    ...
    !hasSeed;
  const initialLaunchpadView = hasExplicitLaunchpadView
  ...
  const resumeSessionSummary = shouldReadResumeSessionSummary
    ...
    : null;
  const mobileResumeDocumentKey = shouldSuppressExampleResume
    ? ""
  ...
  const projectForInitialDocument =
    ...
    null;
  if (
    projectForInitialDocument?.projectKey &&
    projectForInitialDocument.projectKey !== initialProject?.projectKey
  ) {
  ...
  return {
    ...
    getReceiptsConnectionStatus:
      readerData?.getReceiptsConnection?.status || "DISCONNECTED",
    getReceiptsConnectionLastError:
      readerData?.getReceiptsConnection?.lastError || "",
    initialDocument,
    initialProjectKey: initialProject?.projectKey || null,
    initialMode: defaultMode,
    voiceCatalog,
    defaultVoiceChoice: voiceCatalog[0] || null,
    showLaunchpadInitially,
    initialLaunchpadView,
