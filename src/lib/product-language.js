export const COMPANY_NAME = "Lakin AI Inc.";
export const PRODUCT_NAME = "Loegos";
export const PRODUCT_MARK = "Lœgos";
export const BRAND_TRUTH = "Meaning is an assembled object.";
export const ACTION_LINE = "Close the gap between what you think and what you do.";
export const PRODUCT_DESCRIPTOR = "Source to seed to receipt.";

export const HOME_LOOP = Object.freeze({
  speak: "Speak",
  listen: "Listen",
  assemble: "Shape",
  drop: "Drop anything",
});

export const HOME_LOOP_TITLE = `${HOME_LOOP.speak}. ${HOME_LOOP.listen}. ${HOME_LOOP.assemble}. ${HOME_LOOP.drop}.`;
export const METADATA_DESCRIPTION = `${PRODUCT_NAME} is a desktop-first workbench for source to seed to receipt.`;
