...
const geometryCompression = [
  ...
  },
  {
    ...
    body:
      "Hexagonal vessels are reserved for things that have crossed enough of the seven-stage gradient to travel: converged objects, durable receipts, and portable proof. It is not a fifth phase and it is not identical to Seal. It is a settlement condition.",
    tone: "green",
  },
];
...
function SettlementHex({ stage = 0, tone = "blue", size = 88, glyph = "□", label }) {
  const cx = size / 2;
  const cy = size / 2;
  const outerRadius = size * 0.42;
  const innerRadius = size * 0.26;
  const outer = hexPoints(cx, cy, outerRadius);
  const inner = hexPoints(cx, cy, innerRadius);
  const c = toneStyle(tone);
  const clampedStage = Math.max(0, Math.min(stage, 7));

  return (
    <div style={{ display: "grid", gap: 8, justifyItems: "center" }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: "block" }}>
        {outer.map((point, index) => {
          const next = outer[(index + 1) % 6];
          const active = clampedStage > index;
          return (
            <line
              key={`edge-${index}`}
              x1={point[0]}
        ...
        <polygon
          points={inner.map((point) => point.join(",")).join(" ")}
          fill={clampedStage === 7 ? c.bg : "rgba(255,255,255,0.02)"}
          stroke={clampedStage === 7 ? c.color : "rgba(255,255,255,0.08)"}
          strokeWidth={1.2}
        />
        ...
        <text
          ...
          fill={clampedStage === 7 ? c.color : t.sec}
          fontSize={size * 0.22}
          fontFamily={t.sans}
          fontWeight={glyph === "œ" ? 700 : 500}
        >
          {glyph}
        </text>
      </svg>
...
function ConvergenceBar({ percent = 0, width = 172 }) {
  const left = (width / 2) * (percent / 100);
  const right = left;
  const settled = percent >= 88;

  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
      <span style={{ color: t.blue, fontFamily: t.mono, fontSize: 10 }}>△</span>
      <div
        style={{
          width,
          height: 6,
        ...
        {settled ? (
          <div
            style={{
              ...
              borderRadius: 999,
              background: t.amber,
              boxShadow: `0 0 0 3px ${t.shell}`,
            }}
          />
        ) : null}
      </div>
      <span style={{ color: t.green, fontFamily: t.mono, fontSize: 10 }}>□</span>
      {settled ? <span style={{ color: t.amber, fontSize: 12, fontWeight: 700 }}>œ</span> : null}
    </div>
  );
}

function ShapeDot({ tone = "neutral" }) {
  return (
    <span
      style={{
        width: 8,
...
function OperatorBlock({ shape, text, tone, stage }) {
  const s = shapes.find((item) => item.id === shape);
  return (
    <div
      style={{
        display: "grid",
    ...
    >
      <span
        style={{
          ...
          fontWeight: shape === "weld" ? 700 : 500,
          marginTop: 2,
        }}
      >
        {s?.glyph}
      </span>
      <p style={{ margin: 0, color: t.sec, fontSize: 13, lineHeight: 1.55 }}>{text}</p>
      <SettlementHex stage={stage} tone={tone} glyph={s?.glyph || "□"} size={38} />
    </div>
  );
}

function AssembledCard({ shape, title, description, tone, stage, meta, percent, blocks = [] }) {
  const [open, setOpen] = useState(false);
  const s = shapes.find((item) => item.id === shape);

  return (
    <Surface style={{ overflow: "hidden", borderRadius: 20, borderColor: toneStyle(tone).border }}>
      <button
        type="button"
      ...
      >
        <SettlementHex stage={stage} tone={tone} glyph={s?.glyph || "□"} size={62} />
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <Kicker style={{ color: toneStyle(tone).color }}>{s?.label}</Kicker>
            <SignalChip tone={tone}>{stage} / 7 settlement</SignalChip>
          </div>
          <h3 style={{ margin: "10px 0 0", fontSize: 20, lineHeight: 1.18 }}>{title}</h3>
          <p style={{ margin: "10px 0 0", color: t.sec, fontSize: 14, lineHeight: 1.58 }}>{description}</p>
          <div style={{ marginTop: 12 }}>
            <ConvergenceBar percent={percent} />
          </div>
          <div style={{ marginTop: 10, color: t.muted, fontFamily: t.mono, fontSize: 10 }}>{meta}</div>
        </div>
        <div style={{ display: "grid", justifyItems: "end", gap: 10 }}>
          <span style={{ color: t.muted, fontFamily: t.mono, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase" }}>
            {blocks.length} blocks
          </span>
          <span style={{ color: toneStyle(tone).color, fontFamily: t.mono, fontSize: 12 }}>{open ? "▴" : "▾"}</span>
        </div>
      </button>

      {open ? (
        <div style={{ padding: "0 18px 18px", borderTop: `1px solid ${t.line}`, display: "grid", gap: 10 }}>
          <div style={{ paddingTop: 14 }}>
            <Kicker>Operator blocks</Kicker>
          </div>
          {blocks.map((block) => (
            <OperatorBlock
              key={block.text}
...
export default function LoegosFinalProposalV12() {
  return (
    <div
      style={{
        minHeight: "100vh",
      ...
      <div className="final-proposal-shell">
        ...
        >
          <div style={{ maxWidth: 880 }}>
            <Kicker>Final proposal v1.2</Kicker>
            <h1
              style={{
                margin: "8px 0 0",
            ...
            >
              This is the resolved proposal. The shape is the noun. The verb is what you do inside
              it. `Aim / Reality / Weld / Seal` are the rooms the user navigates through, and each
              room carries its own toolset. Capture lives inside Reality. Create and Operate live
              inside Weld. Proof belongs to Seal. The UI now matches the product and the philosophy:
              the box contains operations, just as each shape contains verbs.
            </p>
          </div>

          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <SignalChip tone="blue">shape = noun</SignalChip>
            <SignalChip tone="amber">verb = toolset</SignalChip>
            <SignalChip tone="blue">navigate by noun</SignalChip>
            <SignalChip tone="amber">act by verb</SignalChip>
            <SignalChip tone="amber">6 edges + center = seven</SignalChip>
            ...
            <div className="final-hero">
              ...
              <div className="final-hero-body">
                <div style={{ alignSelf: "end", maxWidth: 620 }}>
                  ...
                  >
                    Four rooms. Four toolsets. The user always knows where they are from the shape
                    and what they can do from the verbs inside it. The shell defaults to square
                    field logic, the user moves by vectors, and settled objects earn a hexagonal
                    vessel only when the seven-stage gradient has done enough work. A receipt can be
                    sealed before an object reaches full seven-end settlement, which keeps Seal and
                    Seven correctly distinct.
                  </p>
                </div>

                <div style={{ display: "grid", gap: 12 }}>
                  {[
                    { label: "Declared", shape: "aim", tone: "neutral", stage: 1, chip: "0 receipts" },
                    { label: "Grounded", shape: "weld", tone: "amber", stage: 4, chip: "3 receipts" },
                    { label: "Portable", shape: "seal", tone: "green", stage: 7, chip: "14 receipts" },
                  ].map((state) => (
                    <Surface
                      key={state.label}
                      style={{
                    ...
                    >
                      <div style={{ width: 108, display: "flex", justifyContent: "center" }}>
                        <SettlementHex
                          stage={state.stage}
                          tone={state.tone === "neutral" ? "blue" : state.tone}
                          glyph={shapes.find((shape) => shape.id === state.shape)?.glyph || "□"}
                          size={94}
                        />
                      </div>
                      <div>
                        ...
                        <p style={{ margin: "12px 0 0", color: t.sec, fontSize: 14, lineHeight: 1.58 }}>
                          {state.label === "Declared"
                            ...
                            : state.label === "Grounded"
                              ...
                              : "Enough of the gradient has been crossed for the object to travel as proof."}
                        </p>
                        <div style={{ marginTop: 12 }}>
                          <div
                            style={{
                          ...
                          <SevenGradient active={state.stage} tone={state.label === "Portable" ? "green" : "blue"} />
                        </div>
                      </div>
                    </Surface>
                  ))}
        ...
        <section style={{ marginTop: 28 }}>
          <SectionHeader
            eyebrow="Assembled cards"
            title="The proposal now carries live primitives instead of only describing them"
            body="Cards can open into operator blocks. The settlement hex shows seven-resolution. The convergence bar shows Aim meeting Reality. This is where the system stops being a poster."
          />
          <div className="final-grid-2">
            <AssembledCard
              shape="reality"
              ...
              stage={5}
              percent={86}
              title="Roadside storefront thread"
              description="Reality has enough weight to shape the object, but not enough yet to travel as portable proof."
              meta="capture on phone · 5 sources · 2 witness returns"
              blocks={[
                {
                  shape: "reality",
                  tone: "green",
                  stage: 2,
                  text: "Photo captured from the car. Frontage and pedestrian angle visible.",
                },
                {
                  shape: "aim",
                  tone: "blue",
                  stage: 1,
                  text: "Does this location fit the declared bakery object or distort it?",
                },
                {
                  shape: "weld",
                  tone: "amber",
                  stage: 5,
                  text: "Lease cost and foot-traffic now converge into a lender-shortlist decision.",
                },
              ]}
            />
            ...
            <AssembledCard
              ...
              stage={7}
              percent={94}
              title="Cost estimate receipt"
              description="Seal owns proof. Seven is the resolution process that let the object earn full portability."
              meta="sealed Mar 8 · 14 receipts · portable outward"
              blocks={[
                {
                  shape: "reality",
                  tone: "green",
                  stage: 4,
                  text: "Three external quotes converged within acceptable range.",
                },
                {
                  shape: "weld",
                  tone: "amber",
                  stage: 6,
                  text: "Operate collapsed variance into one stable startup-cost sentence.",
                },
                {
                  shape: "seal",
                  tone: "green",
                  stage: 7,
                  text: "Receipt sealed and now usable outside the box as evidence.",
                },
              ]}
            />
          </div>
          ...
          <div className="final-grid-2">
            ...
            <Surface style={{ padding: 18 }}>
              ...
              <p style={{ margin: "12px 0 0", color: t.sec, fontSize: 14, lineHeight: 1.62 }}>
                ...
                maintaining, preparing, or arriving. The settlement hex carries that as `6 edges + center`.
              </p>
        ...
        <section style={{ marginTop: 28 }}>
          ...
          <div className="final-grid-2">
            <Surface style={{ padding: 18 }}>
              ...
              <Surface style={{ padding: 14, marginTop: 14, borderRadius: 28, maxWidth: 320 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                  <Kicker>Reality</Kicker>
                  <SettlementHex stage={3} tone="green" glyph="□" size={42} />
                </div>
                <div
                  style={{
                    marginTop: 14,
                ...
                >
                  <div style={{ textAlign: "center" }}>
                    <span style={{ display: "block", fontSize: 30, color: t.blue }}>□</span>
                    <span style={{ color: t.muted, fontFamily: t.mono, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                      capture frame
                    </span>
                  </div>
                </div>
                <p style={{ margin: "12px 0 0", color: t.sec, fontSize: 13, lineHeight: 1.55 }}>
                  Roadside storefront captured first. Classification happens after intake, not before.
                </p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
                  <SignalChip tone="green">Reality</SignalChip>
                  <SignalChip tone="blue">Aim check</SignalChip>
                  <SignalChip tone="amber">Send to Weld</SignalChip>
                </div>
              </Surface>
            ...
            <Surface style={{ padding: 18 }}>
              ...
              <Surface style={{ padding: 14, marginTop: 14 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 12, alignItems: "center" }}>
                  <div>
                    <Kicker>Aim</Kicker>
                    <p style={{ margin: "8px 0 0", color: t.sec, fontSize: 13, lineHeight: 1.5 }}>
                      Seed remains visible while evidence is shaped.
                    </p>
                  </div>
                  <SettlementHex stage={5} tone="amber" glyph="œ" size={60} />
                  <div>
                    <Kicker>Reality</Kicker>
                    <p style={{ margin: "8px 0 0", color: t.sec, fontSize: 13, lineHeight: 1.5 }}>
                      Operate and staging share one convergence field.
                    </p>
                  </div>
                </div>
                <div style={{ marginTop: 14 }}>
                  <ConvergenceBar percent={92} />
                </div>
              </Surface>
              <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
              ...
              <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                >
                  <span style={{ fontSize: 13, lineHeight: 1 }}>œ</span>
            ...
            <div className="final-grid-3">
                ...
                <Surface style={{ padding: 18, marginTop: 18 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, marginTop: 16, flexWrap: "wrap" }}>
                    ...
                    <div style={{ width: 140, display: "flex", justifyContent: "center" }}>
                      <SettlementHex stage={5} tone="amber" glyph="œ" size={112} />
                    </div>
                  </div>

                  <div style={{ marginTop: 14 }}>
                    <ConvergenceBar percent={91} />
                  </div>
              ...
              <Surface style={{ padding: 18, minHeight: 440 }}>
                ...
                <div style={{ display: "grid", gap: 10, marginTop: 20 }}>
                  ...
                  >
                    <span style={{ fontSize: 13, lineHeight: 1 }}>𒐛</span>
                    shareable receipt
                  </div>
                </div>
              </Surface>
