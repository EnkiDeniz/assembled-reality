...
### Toggle: plain mode (option, never default)

The user can toggle to plain mode, but plain mode is an option, not a
starting state. The default for any block that has entered the system
is Lœgos rendering — shape, signal, weight.

The only moment plain text is the native view is while the user is
actively typing in an input field. The characters they are composing
show as they type. The instant the text enters the system as a block,
it renders in Lœgos language.

The principle: text is already coordination code. Plain text is the
hidden state, not the natural one. Loegos rendering is the act of
reading the code that was always there. Showing plain text first and
then "turning on" Lœgos would be like showing source code without
syntax highlighting and then enabling it. The highlighting is not a
view mode. It is the language.

### The mystery unfolds

A freshly pasted source does not arrive with strong opinions. It
arrives with shape glyphs inferred from content, neutral signal, and
light weight. The structure is visible. The opinion is not yet formed.

When the user runs Operate, signal colors appear — green where
evidence grounds the claim, amber where evidence is partial, red where
nothing supports it. When the user adds evidence or attests a block,
weight increases. When reality returns and a seal happens, weight
locks into strong.

This is not a staged rollout of the language. It is the language
behaving correctly. The user discovers what is grounded through
interaction, not through a tutorial. Consent before compute (see
first-principles.md) means the system does not form opinions until
the human declares.
